...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        digitsOnly = TextUtils.isDigitsOnly(input.getText().toString());
        alert.setView(input);
    ...
    private boolean validateFields() {
        if (digitsOnly == false) {
            input.setError("Your Input is Invalid");
            return false;
        } else {
            return true;
        }
    ...
    **/
    ...
    public void display(EditText in) {
        ConstraintLayout w = findViewById(R.id.whole);
        w.setVisibility(View.VISIBLE);
        size = Double.parseDouble(in.getText().toString());
